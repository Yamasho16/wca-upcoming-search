...
Vercel 運用を想定し、同期はページ単位で実行します。

```bash
curl -X POST "http://localhost:3000/api/admin/sync?page=1" \
...
```bash
curl "http://localhost:3000/api/admin/sync?page=1&secret=replace-with-random-secret"
```

レスポンスの `hasNextPage` と `nextPage` を見て、次ページを順に叩きます。最後のページ完了後に `finalize=1` を付けて 1 回実行すると、今後開催一覧から消えた大会を `isUpcoming=false` に更新します。

```bash
curl -X POST "http://localhost:3000/api/admin/sync?page=24&finalize=1" \
  -H "x-sync-secret: replace-with-random-secret"
```
...
1. `page=1` から同期開始
2. `hasNextPage=true` の間は `nextPage` を順に実行
3. 最後のページをもう一度 `finalize=1` 付きで実行
...
- WCA 側負荷を下げるため、同期は既定で `SYNC_CONCURRENCY=2`、バッチ間に `SYNC_BATCH_DELAY_MS=750` の待機を入れる
