...
```bash
curl -X POST "http://localhost:3000/api/admin/sync?page=1&offset=0&chunkSize=5" \
...
```bash
curl "http://localhost:3000/api/admin/sync?page=1&offset=0&chunkSize=5&secret=replace-with-random-secret"
```
...
```bash
curl -X POST "http://localhost:3000/api/admin/sync?page=24&offset=0&chunkSize=5&finalize=1&finalizeOnly=1" \
  -H "x-sync-secret: replace-with-random-secret"
```
...
- 1 回の Vercel 実行時間を短く保つため、既定で `SYNC_CHUNK_SIZE=5` のチャンク単位同期を行う
